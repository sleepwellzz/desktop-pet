...
## 14. 交互模型修订（2026-09-17）——本文档 §3.1 与 §4 **已被取代**

用户人工复验后拍板的新模型，详见 **ADR 016**。与本文档的偏差：

| 本文档 | 现状 |
|---|---|
| §3.1 双路径显示：悬停 `showInactive` / 快捷键 `show` | **悬停整条路径删除**；三条唤出路径（右键宠物 / `Win+Alt+P` / 托盘菜单）**一律抢焦点** |
| §3.2 基线 2 的"悬停出现"（D3） | 不做。理由：光标路过宠物是高频的顺便事件，自动弹出必然变成"我没叫它它自己冒出来"，延迟只降频不改性质；且左键单击要播动作，"停在它身上"不该带着第二个含义 |
| §4 状态机（含 `armed` / `suppressUntil` / 悬停计时） | 只剩五条规则；那两个补丁位连同机制删除（它们要防的场景是悬停专有的） |
| §2 面板内容：动作排含缩放 − / + / ↺ 与只读百分比 | 上一轮撤掉按钮，本轮把只读的 `70%` 换成**宠物名**（取 `pet.json` 的 `displayName`）；缩放只在菜单里改 |
| §3.3 `hide()`→`show()` 的键盘路径结论 | 不变（实测仍然不受影响，不需要 reload） |

新增的一条机制（本文档没预见的）：**`⋯` 弹原生菜单期间必须屏蔽"失焦即收"** ——
原生菜单会夺走弹出它的窗口的焦点，不屏蔽的话面板会在菜单弹出的同一瞬间吃到 `blur`，
200ms 后连菜单一起消失（"⋯ 点了没反应"）。见 ADR 016 第 6 条（含"哪部分是实测、哪部分是推断"的标注）。

§12「本设计不做的事」里"不改单击宠物 = 确认的既有语义"仍然成立，但**语义扩容了**：
单击现在明确是"播一次「拜一下」（第 3 行 `waving`）+ 确认"（ADR 016 第 5 条）。
