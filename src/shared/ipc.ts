...
export const CH = {
   ...
   * 渲染层 → 主进程：在宠物实体上按下了右键。
   *
   * 语义在 2026-09-17 改了（ADR 016）：**唤出/收起控制条**，不再是弹出原生菜单。
   * 触发点必须在渲染层：窗口常态整窗穿透、命中与否由渲染层的 alpha 采样决定（ADR 008），
   * 只有它知道这一下右键落在精灵轮廓上还是空白处。主进程只负责切换面板，不判断命中。
   * 不带坐标：面板的位置由"贴着宠物"这个规则算出来，不由光标决定。
   */
  contextMenu: 'pet:context-menu',
  /**
   * 主进程 → 控制条：整份视图数据（状态、会话列表、宠物名…）。
   * 控制条是**只读面板**：它自己不持有状态、不读文件，所有内容都从这里来。
   */
  barView: 'pet:bar-view',
  /**
   * 主进程 → 控制条：请聚焦首屏（右键 / 菜单唤出时用）。
   *
   * 为什么焦点要由主进程下令而不是渲染层自作主张：渲染层看不到"我是怎么被唤出来的"。
   * （历史上曾有"悬停 = 不抢焦点 / 快捷键 = 抢焦点"两条，悬停已删于 ADR 016、快捷键已删于 ADR 032，
   *   现在只剩"一律抢焦点"一种 —— 通道保留，因为面板将来可能有不抢焦点的唤出场景。）
   */
...
export interface BarView {
  ...
  maxRows: number;
  /**
   * 面板上显示的名字（宠物包 manifest 的 `displayName`）。
   *
   * 2026-09-17 换掉了原来的"当前缩放百分比"：面板是**状态仪表盘**，
   * 一个只读的 `70%` 既不能点、也不是用户关心的事（缩放有托盘/右键菜单里的档位），
   * 而"这块面板是谁的"才是它该回答的第一个问题。缩放操作不再出现在面板上。
   */
  petName: string;
